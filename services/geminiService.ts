...
/**
 * Analyzes a scene image to determine the best plant size and placement location
 * @param environmentImage The file for the background environment to analyze
 * @returns A promise that resolves to an object containing the recommended plant size and placement coordinates
 */
export const analyzeSceneForPlantPlacement = async (
  environmentImage: File
): Promise<{ 
  recommendedSize: 'small' | 'medium' | 'large' | 'huge';
  placementPosition: { xPercent: number; yPercent: number };
  reasoning: string;
}> => {
  console.log('Analyzing scene for optimal plant placement...');
  ...
  You are an expert interior designer and space analyst. Analyze this room image and determine the optimal plant placement.

  PLANT SIZE GUIDELINES (based on Easyplant specifications):
  - SMALL (9"-16" tall, 5-6" pot): For tables, desks, small surfaces, narrow spaces
  - MEDIUM (11"-26" tall, 7-8" pot): For regular floor spaces, medium surfaces, standard rooms
  - LARGE (23"-40" tall, 11" pot): For big empty spaces, large rooms, open areas
  - HUGE (40"-69" tall, 11" pot): For very large empty spaces, high ceilings, grand rooms

  PLACEMENT RULES:
  - AVOID: In front of computer screens, monitors, or TVs
  - AVOID: Blocking doorways, entrances, or walkways
  - AVOID: In front of windows (blocks light)
  - AVOID: On fragile surfaces or valuable furniture
  - PREFER: Floor spaces, sturdy tables, corners, empty walls
  - PREFER: Areas with good lighting but not direct sunlight
  - PREFER: Spaces that complement the room's design

  ANALYSIS TASKS:
  1. Identify available surfaces (tables, floors, shelves, counters)
  2. Assess room size and ceiling height
  3. Look for screens, doors, windows, and walkways to avoid
  4. Determine the most appropriate plant size based on available space
  5. Find the best placement location that follows the rules above

  Respond with a JSON object in this exact format:
  {
    "recommendedSize": "small|medium|large|huge",
    "placementPosition": {"xPercent": 50, "yPercent": 70},
    "reasoning": "Brief explanation of why this size and location was chosen"
  }
  `;
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: { parts: [{ text: analysisPrompt }, environmentImagePart] }
    });

    const responseText = response.text;
    console.log('Scene analysis response:', responseText);
    ...
    return {
      recommendedSize: analysis.recommendedSize || 'medium',
      placementPosition: analysis.placementPosition || { xPercent: 50, yPercent: 70 },
      reasoning: analysis.reasoning || 'Default placement selected'
    };
  } catch (error) {
    console.error('Failed to analyze scene:', error);
    // Fallback to medium size in center
    return {
